        ...
        @if(!empty($hDetails))
            <div class="inline-block  align-middle ">
                <span class="ml-2 h-full align-middle text-base">
                    Name : {{ $hDetails->Sei." ".$hDetails->Mei }}
                </span>
            </div>
        @endif
    </div>
        ...
        <ul class="w-96 bg-gray-50 drop-shadow-lg">
            @if(!empty($records))
                @foreach($records as $record)
                    <li class="p-2 text-base hover:bg-gray-400 hover:text-white" wire:click="fetchDetail('{{ $record->HogoshaCd }}')">{{ $record->HogoshaCd}} , {{ $record->Sei }} , {{ $record->Mei}}</li>
                @endforeach
            @endif
        </ul>
    </div>
