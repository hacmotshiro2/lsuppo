...
class LCoinBalanceNotification extends LSuppoNotification
{
    use Queueable;
     ...
     * @return void
     */
    public function __construct($isToHogosha = false, $name, $students, $balancePerStudent)
    {
    ...
    public function toMail($notifiable)
    {
        ...
        if($this->toHogosha){
            //保護者向けの場合
            $ret = $ret
            ->subject(env('MAIL_SUBJECT_PRE').'エルコイン残高をお知らせします')
            ->greeting($this->name.'さん こんにちは　プログラミングスクールLです。')
            ->line('')
            ->line($today.'時点のエルコイン残高をお知らせします。');
        }
        else{
            //サポーター向け（管理者向け）の場合
            //序文
            $ret = $ret
            ->subject(env('MAIL_SUBJECT_PRE').'エルコイン残高の一覧をお知らせします')
            ->greeting($this->name.'さん こんにちは　プログラミングスクールLです。')
            ->line('')
            ->line($today.'時点のエルコイン残高の一覧をお知らせします。');
        }
        //中身
        foreach($this->balancePerStudent as $key => $balance){
